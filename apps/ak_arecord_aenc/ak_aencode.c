...
// Retired from ak3918ev300v18/platform/libmpi/demo/aenc_demo/ak_aenc_demo.c
...
// note (Ricardo): this is used to not compile some code incompatible with libraries on camera
// I'm using the original libraries, as I still haven't been able to run the manually compiled ones
#define NOT_WORKS_ON_V380CAM 1
...
int main(int argc, char **argv)
{
	if (argc != 8) {
		printf("usage: %s [sample rate] [channel] [type] [save path] [time(secs)] [volume] [mic/linein]\n",
				argv[0]);
		printf("\033[40;33m eg.: %s 8000 1 mp3 /mnt/sdcard/tmp 10 7 mic \033[0m\n", argv[0]);
